...
### 1、网络模型
#### 应用层（表示层，会话层）
- 核心协议： HTTP/HTTPS (Web)，DNS (域名解析，走 UDP)，SSH (远程登录)
...
- NAT：路由器实现出关改源 ip，入关改目标 ip（nginx）（路由器会跟踪客户端的 TIME_WAIT，除非端口不够）
- ARP：ip 映射 mac，没有则广播
...
- HTTPS：
	...
	- 会话复用：
		...
		- 在 1.3 的重连中，客户端会把 Ticket 和 HTTP 请求一同发送给服务端，实现 0 握手，但 0 RRT 会有重放攻击的危险，即第一次请求是非幂等的，黑客拦截请求然后重复发（黑客如果利用同一个 tcp 连接拦截不是第一次的请求重复发，会因为隐式序列号对应不上而失败）
...
- 滑动窗口：大小由接收方决定，分为 swnd 和 rwnd
	...
	- 在 swnd 归 0 后，发送方会定时发一个窗口探测，询问 rwnd 有没有变化，接收方也可以主动通知，但 ack 不一定到达
...
- challenge ack：
	...
	- RST：收到相同四元组的序列号在接收窗口内的 RST，回复 challenge ack，断开方拿到确切的序列号后再发 RST，如果在接收窗口外的话，认为是 RST 盲打攻击直接忽略（在默认情况下，在 TIME_WAIT 状态中的连接是可以被 RST 关闭的，但是也有个参数 net.ipv4.tcp_rfc1337 开启后可以无视）
...
- address_space：由每个 Inode 维护，记录着文件偏移量和物理内存地址的映射关系，通过 Radix Tree 基数树去维护，如果找不到物理内存地址就只能通过 Inode 指向的数据块从磁盘中读，如果是用 mmap() 映射的文件则加入进程的页表，后续通过页表读就行了
### 4、内存管理
- 虚拟内存：
...
- malloc()：
	...
	- mmap() 带上私有匿名映射参数：申请大内存，在堆中单独拉一块区域，free() 后释放（具名映射是将文件映射到用户虚拟地址空间但加载到 pagecache 中，当然同样是缺页中断时才加载，如果是私有的还会写时复制到用户物理地址空间）
- 回收机制：
...
- 进程间通信：
	...
	- 信号：在 task_struct 的 pending 位上，当进程从内核态返回前检查信号变化
		- 执行默认操作
		- 捕捉信号，前提是注册了信号的处理函数
		- 忽略信号，除了`SIGKILL` 和 `SEGSTOP`，它们必须强制执行
...
- IO 多路复用：
	...
	- poll：使用包含结构体的数组
	- epoll：使用红黑树注册 + 双向链表存就绪事件，使用 epoll_create() 初始化，epoll_ctl() 增删改节点，epoll_wait() 查看就绪事件（没有事件可以阻塞也可以返回错误）
		- 为什么是红黑树：hash 不好扩缩容，链表不好增删改时的查找
		- 水平触发（_level-triggered，LT_）：直到缓冲区数据被读完，调用 epoll_wait() 才没反应
...
- reactor：
	...
	- 多 reactor 多线程：主 reactor 进行 accept()，再给多个从 reactor 去 epoll_create()，每个从 reactor 都有一套 epoll，然后再给多个 worker 处理，这样做可以减轻每个 reactor 的 epoll_wait() 压力
- proactor：程序发起异步请求，同时递交一个内存缓存区，操作系统把数据运到这个缓冲区，并通知程序
- qdisc（Queueing Disciplines，排队规则）：
	- 内核在内存中维护的软件队列，在 Ring Buffer 之上，有队列长度 txqueuelen，太长会导致缓冲区膨胀
	- pfifo_fast (默认，优先先出)，fq_codel (Fair Queuing Controlled Delay)，fq (Fair Queue，为每个 TCP 流量流维护一个单独的队列，需要启用 TCP BBR 拥塞控制算法来计算带宽延迟乘积)
### 7、调度算法
- 进程调度：
	- 先来先服务：短作业被长作业阻塞
...
### 12、排序
![](../picture/八股文/file-20260118002805611.png)
### 13、其他
...
- cache
	...
	- 缓存一致性：
		...
		- 伪共享：多个核心操作了物理内存相邻的数据，解决：数据填充，冷热数据分离，线程局部变量分离
### 5、硬盘
- 机械硬盘 HDD
- 固态硬盘 SSD
- 盘面，磁道，扇区（512B 或 4kB）
...
- group by：如果不加索引的话会触发哈希聚合（老版本 mysql 是排序聚合）
- count：
...
### 2、线程模型
- 后台线程：close_file，sof_fsync，lazy_free（删除大键时，交给它异步释放内存），io_thd（读取数据并解析成 redis 命令）
...
- 混合持久化：全量 RDB + 增量 AOF
### 4、过期删除
- dict（全局字典）和expires（过期字典，存的是设置了过期时间的数据）
...
- Cache Aside（旁路缓存）
	- 先删再改：A 线程先把数据从缓存里删了，但在改数据库前，B 线程过来把数据库的数据写到了缓存，发生概率较大
	- 先改再删：A 线程先改了数据库里的数据，但在删缓存前，B 线程过来把数据库的数据改了发生概率较小
	- 需要使用缓存双删，或者异步删除的策略
...
### 8、其他
- 为什么大 key 不好：get all 的时候会阻塞主线程和网络，集群模式下的内存倾斜和请求倾斜，重写日志时大 key 的增删改会牵动大量的数据页导致写时复制开销巨增
## 七、语言
### Java
#### 1、基础
- `volatile`：变量写完后必须强制写回内存，读时也必须从内存中拿，虽然留在缓存里也可以通过 MESI 协议共享，但性能较差
- `synchronized`：对代码块上锁，且要传入一个对象，本质就是在 Object 头里有一块区域可以打标记
...
- 创建 G：被放在 runnext 里`->`runnext 里的 G 被挤到本地 P 中`->`本地 P 也满了就放到全局 G 队列中
...
### 5、git
- blob，tree，commit
- add，commit，push，pull，fetch，reset，revert，diff，switch（checkout），restore，stash，rm，branch
### 6、DMCA 
- 保护版权
### 7、灰度、滚动、蓝绿发布
- 灰度：重点是流量的分配
...
### 13、linux 常见命令
- grep，echo，lsof，ss，chmod，free，top，ps，mkdir，touch，rm，cat，head，tail，mv，tar，sed，awk，wc，ping，curl
### 14、docker
